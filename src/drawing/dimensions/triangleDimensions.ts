// src/drawing/dimensions/triangleDimensions.ts
...
export function createTriangleDimensions(triangleMesh: THREE.Mesh) {
  
  const arrowStyle = getArrowStyle();
  const widthOffsetDistance = getShapeWidthOffset(triangleMesh);
  const heightOffsetDistance = getShapeHeightOffset(triangleMesh);

  triangleMesh.geometry.computeBoundingBox();
  const boundingBox = triangleMesh.geometry.boundingBox;
  ...
  const bottomLeftWorld = bottomLeftLocal.clone().applyMatrix4(triangleMesh.matrixWorld);
  const bottomRightWorld = bottomRightLocal.clone().applyMatrix4(triangleMesh.matrixWorld);
  const topLeftWorld = topLeftLocal.clone().applyMatrix4(triangleMesh.matrixWorld);
