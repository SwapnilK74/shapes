// src/drawing/dimensions/planeDimensions.ts
...
export function createPlaneDimensions(planeMesh: THREE.Mesh) {
  
  const arrowStyle = getArrowStyle();
  const widthOffsetDistance = getShapeWidthOffset(planeMesh);
  const heightOffsetDistance = getShapeHeightOffset(planeMesh);

 
  planeMesh.geometry.computeBoundingBox();
  const boundingBox = planeMesh.geometry.boundingBox;
  ...
  const bottomLeftWorld = bottomLeftLocal.clone().applyMatrix4(planeMesh.matrixWorld);
  const bottomRightWorld = bottomRightLocal.clone().applyMatrix4(planeMesh.matrixWorld);
  const topLeftWorld = topLeftLocal.clone().applyMatrix4(planeMesh.matrixWorld);
